...
export default function All() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("https://oprahstudio-portfolio-react-backend.vercel.app/projects");
        ...
        <div>
            <div className="project mx-9 sm:px-10 mb-10 max-w-screen">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 lg:grid-cols-2 gap-5 mt-2">
                    {project.map((item) => (
                        <div id="project_type" class="mt-3 bg-white rounded-lg h-fit hover:drop-shadow-xl" key={item}>
                            ...
                            <div class="px-5 py-4">
                                <a href={`/projects/${item.project_id}`} class="project-title text-3xl font-ed text-dusk-yellow">
                                    {item.name}
                                </a>
